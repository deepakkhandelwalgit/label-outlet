 ...
 */
function retailplus_commerce_install_tasks_alter(&$tasks, $install_state) {
	...
	$tasks['retailplus_commerce_content'] = array(
 ...
 */
function retailplus_commerce_form_install_configure_form_alter(&$form, $form_state) {
	$form['site_information']['site_name']['#default_value'] = 'Retail+';
    $form['#submit'][] = 'retailplus_commerce_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function retailplus_commerce_install_configure_form_submit($form, &$form_state) {
	global $user;
	...
	theme_enable(array('retailplus', 'seven'));
	variable_set('theme_default', 'retailplus');
	variable_set('admin_theme', 'seven');
	variable_set('file_temporary_path', 'sites/default/files/tmp');
	
	drupal_set_message("Retail+ Commerce Easy Installation was a Success!");

}

function retailplus_commerce_content(&$install_state) {
	$sql_file=dirname(__FILE__).'/retailplus_commerce.sql';
	$count=retailplus_commerce_import_sql($sql_file);
}

function retailplus_commerce_import_sql($filename){
