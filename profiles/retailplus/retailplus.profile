 ...
 */
function retailplus_install_tasks_alter(&$tasks, $install_state) {
	...
	$tasks['retailplus_content'] = array(
 ...
 */
function retailplus_form_install_configure_form_alter(&$form, $form_state) {
	$form['site_information']['site_name']['#default_value'] = 'Retail+';
    $form['#submit'][] = 'retailplus_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function retailplus_install_configure_form_submit($form, &$form_state) {
	global $user;
	...
	drupal_set_message("Retail+ Easy Installation was a Success!");

}

function retailplus_content(&$install_state) {
	$sql_file=dirname(__FILE__).'/retailplus.sql';
	$count=retailplus_import_sql($sql_file);
}

function retailplus_import_sql($filename){
